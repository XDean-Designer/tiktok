...
39. **开通页进团购券 +「共 N 项」累加（v1.6.5 / v1.6.14）**：已开通主按钮「去设置团购券」→ 右侧滑入直达当前平台 `tg-deals`（非 `tg-set`）；标题栏返回回 `tg-set`（团购设置，右侧滑出），再返回回账户页。工作台/无权限「去团购设置」仍进 `tg-set`。「共 N 项」= 已绑门店目录券数**累加**（非去重）。
...
48. **页转与消费顾客必选（v1.6.14）**：扫码→输码改为整页自下方移入移出（无弹性）；手机预览团购券水平推页对齐状态栏高度变量（无假状态栏时为 0），且**推入前按目标平台渲染**（点击抖音即显示抖音列表，不在落位时切换）；确认核销时未选消费顾客不再默认男散客，拦截并对「散客｜会员」双卡做 iOS 式抖动；选择消费项整页自下方移入移出（确认页与团购设置共用）。
49. **开通页进券列表的返回层级（v1.6.14）**：开通页「去设置团购券」进入 `tg-deals` 亦为**右侧滑入**（与团购设置点平台卡一致）；标题栏返回改为回 **`tg-set`（团购设置）**（右侧滑出），`tg-set` 再返回回账户页，开通页不再留在历史栈中。
50. **默认配置保存成功动效（v1.6.14）**：点「保存」后先给 **iOS 式成功反馈**再收起——按钮文字淡出换成白色对勾（`stroke` 描画 0.32s，底色转成功绿 `#2BA471`）、卡片一次 `scale 1→0.985→1` 按压回弹、券卡「已配置」标签由灰渐变绿并做 `0.6→1.08→1` 徽标式弹入（已存匹配为空时不动标签）；停留约 **320ms** 后展开区**先淡出上移 6px 并收拢高度（约 160ms）**，再移除展开态并重渲染列表（避免内容硬切）；保留居中 toast「已保存默认配置」。`prefers-reduced-motion` 降为即时呈现（约 150ms）。
51. **撤销结果页与返回层级（v1.6.14）**：`revoke-ok` 由「撤销成功」升级为**撤销结果（成功 / 失败共页，标题统一「撤销结果」）**，新增三类失败态（网络异常可重试 / 已超时不可撤销 / 已在其它设备撤销），失败态**不改订单状态**，仅成功态冲销并回滚业绩（次卡恢复次数）；撤销结果页的**标题栏返回与「返回核销明细」均回到核销明细**（移除链路上的整页详情与撤销页），**核销明细再返回回到扫码页**（`BACK_PARENT.orders = 'scan'`）；失败态经演示条**单一「撤销失败」入口循环切换**。
...
- 核销明细（优惠券示意 + 抖音列表 + 美团列表 + 筛选；**列表主标题为团购券名**；**列表不展示 refund**；筛选无「已退款」、**无重置**）→ 订单详情（标题=券名 +「核销项目」+ 消费顾客）→ 撤销（1h）→ 撤销结果
...
团购核销（单店 · 抖音/美团）
 ...
 │    └─ 撤销 → revokeMask → revoke-ok（成功）/ revoke-fail（失败，演示）
 ├─ revoke-ok 撤销结果             <!-- #revoke-ok -->
 │    ├─ 成功：开单冲销、业绩回滚（次卡恢复次数）
 │    └─ 失败：网络异常（可重试）/ 已超时不可撤 / 已在其它设备撤销
 │    └─ 「返回核销明细」/ 标题栏返回 → orders（再返回 → scan）
...
| 撤销结果（成功） | `revoke-ok` | 撤销结果 | 开单冲销、业绩回滚；返回核销明细 |
| 撤销结果（失败） | `revoke-fail` | 撤销结果 | 网络异常可重试 / 已超时 / 已在其它设备撤销（演示入口） |
...
- `revokeMask`：二次确认后 loading → 撤销结果页（成功；失败态见 §6.12.2，演示入口）。
...
- 底部「输入券号」→ `input`（须过 `rolePerm`；可带入当前平台）：自扫码进入时整页自**下方移入**（常规平移、**无弹性**）；返回自**下方移出**回扫码；不覆盖状态栏（手机预览无假状态栏时全高）。
...
- **呈现（v1.6.14）**：自扫码「输入券号」进入时整页自下方覆盖移入（无弹性）；其它入口仍可整页进入。
...
- **撤销**：
  - 1h 内：`revokeMask` → 确认 → loading → `revoke-ok`（成功；列表展开区与整页详情共用；失败态见 §6.12.2）；
  - 超时：按钮禁用「已超时不可撤销」；可展示客服指引卡；
  - 已撤销 / 已退款：按钮禁用对应文案（已退款态按钮文案「已退款」）。

## 6.12 撤销结果（成功 / 失败） <!-- #revoke-ok -->

> 成功与失败**共页**（`screen-revoke-ok`，标题统一「撤销结果」），按 `session.revokeRestored` 与失败态渲染；标题栏返回与底部「返回核销明细」均为**回到核销明细**。

### 6.12.1 撤销成功

- 绿色对勾 +「撤销成功」；说明：对应开单已冲销，员工业绩已回滚，撤销流水已保留（订单流水对应开单作废）。
- 次卡撤销会恢复次数：说明追加「已恢复次卡 N 次」，并 toast「已恢复 N 次」。
- 主按钮「返回核销明细」→ `orders`。

### 6.12.2 撤销失败（三类，演示入口「撤销失败」循环切换）

| 失败态 | 标题 | 说明 | 按钮 |
|--------|------|------|------|
| `network` 网络异常 | 撤销失败 | 提交未成功、订单状态未变更（不会重复冲销），检查网络后重试 | 主「重试撤销」+ 次「返回核销明细」 |
| `timeout` 已超时 | 已超时不可撤销 | 已超过平台 1 小时撤销时限，系统不再受理 | 主「返回核销明细」 |
| `revoked` 已在其它设备撤销 | 该核销已撤销 | 已在其它设备撤销（或已退款），无需重复操作、不会重复回滚业绩 | 主「返回核销明细」 |

- 失败态**不改订单状态**（`status` 仍为 `ok`）；仅成功态执行冲销与业绩回滚。
- 「重试撤销」→ loading「正在撤销…」→ 成功后转成功态（同一页）。

### 6.12.3 返回层级

- 撤销结果页（成功或失败）→ **返回核销明细**（`orders`），且会移除链路上的整页详情与撤销页；
- 核销明细再返回 → **扫码**（`scan`；与扫码页「验券订单」入口一致，`BACK_PARENT.orders = 'scan'`）。
...
- **已开通**：授权账号、已绑定门店（可管理）、「取消授权」、主按钮「**去设置团购券**」（含设置 SVG 图标）→ 右侧滑入直达当前平台 `tg-deals`；标题栏返回回 **`tg-set`（团购设置）**（右侧滑出），`tg-set` 再返回回账户页（开通页不留在历史栈）。工作台「团购设置」入口不变。
...
**首页卡片**：已开通「共 N 项」（N = 当前平台**已绑门店**目录 L1 **按店累加**合计，同券多店各计 1；演示绑 3 店时抖音/美团均为 4+3+2=**9**）；未开通「去开通」；已到期「授权已到期，去续期」。点击：非已开通 → `auth-open`（**不**做页转）；已开通 → **先按目标平台渲染列表**，再令 `tg-deals` **从右侧滑入**（约 320ms）；自列表返回 1C 时 `tg-deals` **向右侧滑出**（约 280ms）。`prefers-reduced-motion` 时即时切换。
...
**列表与默认配置（v1.6.11 / v1.6.12 / v1.6.14）**：点券卡在列表内展开。展开区**不重复**列表已有券头信息；仅保留核销项目/单次核销编辑 + 取消/保存（间距 16px）。**次卡**另在展开区以简洁 kv 展示总次数、单次结算价。展开后隐藏顶部配置摘要；划线仅保留「已配置」标签下方一条。取消＝丢弃未保存并收起；保存＝写入 `dealDefaults`，先播 **iOS 式成功反馈**（按钮勾选形变 + 卡片 `scale 1→0.985→1` 回弹 + 「已配置」标签灰→绿弹入，约 320ms），再让展开区淡出上移并收拢高度（约 160ms）后收起；切换其它卡＝丢弃未保存。展开后若卡片下半超出可视区则滚动使卡垂直居中。整页 `tg-config` 导航已废弃（宿主仅作内联挂载）。核销预填优先取**当前登录门店**下该 L1 的默认匹配。撤销后流水作废规则同 v1.5.7。
...
| `revokeMask` | 撤销确认 | 确认后进撤销结果页（成功 / 失败共页，见 6.12） |
...
明细（列表标题=团购券名；过滤 refund）→ 详情（标题=券名 +「核销项目」）→（1h 内）撤销 → revoke-ok（撤销结果，成功/失败共页；返回 → 明细 → 扫码）
```
...
25. **团购设置**：双平台阴影卡片 + Figma 品牌方标；已开通「共 N 项」（已绑门店目录**按店累加**，非去重）；已开通卡进 `tg-deals` 为**右侧水平滑入**（推入前先按目标平台渲染；`--status-bar-h` 对齐，手机预览无假状态栏时为 0）、返回**右侧滑出**；多店时顶 TAB + 切 Tab stagger；列表阴影券卡**内联展开**默认配置；`dealDefaults[storeId][L1].matches:[{id,qty}]`；核销预填取登录门店桶。开通页「去设置团购券」同样右侧滑入 `tg-deals`，返回回 `tg-set`（再返回回账户页）。
26. **核销明细**：Tab stagger；点卡内联展开详情（无「状态」字段）；**次卡**含总次数/核销次数/剩余次数；展开区可撤销。
27. **扫码→输码**：整页自下方移入移出（无弹性）；不覆盖状态栏。
...
**导航/按钮**：扫码、验券订单、输入券号、立即验券、重新扫码、去核销、选择消费项、修改、取消、确认核销、选择会员、重新选择、加入会员、不加入、确定、清空、继续核销、完成、筛选、应用、撤销核销、返回核销明细、重试撤销、查看原单、手动输码、查看订单、去团购设置、去设置团购券、重试核销、我知道了、项目、产品、优惠券、抖音、美团、点客、散客、大工、中工、小工、男、女、会员。
...
**1h 撤销**
14. **Given** 订单在 1 小时内且状态已核销，**When** 详情点「撤销核销」并确认，**Then** 进入撤销结果（撤销成功）；明细状态为已撤销；在结果页点「返回核销明细」或标题栏返回，**Then** 回到核销明细，再返回回到扫码页。
