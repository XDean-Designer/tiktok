...
15. **账号管理入口**：演示首页点**门店名**进入「账号管理」；「团购核销」行右侧抖音/美团分两行热区，展示授权态文案（去开通 / 待确认 / 已授权 / 已到期；后三者字色对齐授权页标签色），点击进入对应授权指引。
...
|------|------|-----------------|----------------------------------|
| `none` | 未发起 | 未发起 | 拦截 → **居中弹窗**「尚未开通抖音团购核销」；「去开通」→ `owner-auth` |
| `pending` | 待确认 | 待确认 | 同 none |
| `ok` | 已授权 | 已授权 | 允许 |
| `expired` | 已到期 | 已到期 | 同 none（弹窗文案仍为尚未开通/引导开通） |

### 4.3.2 美团授权 `authMeituan`（独立于来客）

| 状态 | 含义 | 行为（当前平台=美团） |
|------|------|------------------------|
| 非 `ok` | 未授权 / 待确认 / 到期等 | 拦截 → **居中弹窗**「尚未开通美团团购核销」；「去开通」→ `owner-auth-mt`（占位） |
| `ok` | 已授权 | 允许验券/核销 |
...
- 状态文案：`none`→「去开通」(橙 #FDA446)；`pending`→「待确认」(对齐标签 `#FF4F72`)；`ok`→「已授权」(对齐 `#3AB883`)；`expired`→「已到期」(对齐 `#A9A9A9`)。左侧授权演示条切换后本页同步。
