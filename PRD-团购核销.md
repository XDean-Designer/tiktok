...
**已由 v1.4.2–v1.5.3 原型拍板：** 映射缺失弹窗文案；绑定码 + 已关联（K1）；授权三态 UI；用户端不展示团单/商品 ID；团购设置默认**仅匹配项目+数量**（按 L1）；美团用户端称「团购券」；双平台 seed 目录；**v1.5.1** 品牌方标 SVG、1C/1D/1E 阴影券卡视觉、默认配置去页底预填提示；**v1.5.3** 去掉匹配状态标签、代金券/计次卡不支持提示、筛选无重置、系统券走线上能力示意。
