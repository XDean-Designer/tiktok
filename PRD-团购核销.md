...
店员 → 扫码/输码 → 经汇付验券核销 → 落 L2/L3 → 开单(UI=抖音团购)
```
...
汇付 → 回调/查询 → 剑琅后端还原 store_id → upsert 映射(auth=ok·已关联)
剑琅后端 → 【待汇付确认】拉 L1 团购券列表？→ 可则落目录 / 否则跳过
店员 → 扫码/输码 → 经汇付验券核销 → 落 L2/L3 → 开单(UI=美团团购)
```
...
**已由 v1.4.2–v1.5.4 原型拍板：** 映射缺失弹窗文案；绑定码 + 已关联（K1）；授权三态 UI；用户端不展示团单/商品 ID；团购设置默认**仅匹配项目+数量**（按 L1）；美团用户端称「团购券」；双平台 seed 目录；**v1.5.1** 品牌方标 SVG、1C/1D/1E 阴影券卡视觉、默认配置去页底预填提示；**v1.5.3** 去掉匹配状态标签、代金券/计次卡不支持提示、筛选无重置、系统券走线上能力示意；**v1.5.4** 1C/1D/1E 标题栏返回层级修复。
