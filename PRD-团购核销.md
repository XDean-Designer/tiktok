...
| 确认页消费顾客 | 卡片形变：根态散客｜会员 → 四卡 / 加入会员大卡 / 会员摘要 | 默认可记男散客；会员未选则确认按钮禁用；加入会员：手机号或称呼任一即可建档；称呼空→「尾号」+手机后四位；手机号须 11 位 |
...
| 默认记单 | 根态未展开时仍按散客逻辑：默认性别「男」→「男散客」（可直接确认核销） |
| 点「散客」 | 形变为四卡：左约 62% 并排「男散客｜女散客」，右纵向两枚扁卡「返回」「加入会员」（总高约 84px，右侧**非**强制正方形）；默认选中**男散客**（马卡龙蓝）；女散客为马卡龙粉；二者单选 |
| 点返回 | 形变回根态双卡 |
...
- 字段：金额、状态、券码、平台单号、核销时间、操作人、消费顾客、**核销项目**（次卡标签为「单次核销」；值为匹配消费项名，多项顿号连接，无匹配→「快捷开单」）、开单号、撤销时限提示；平台文案随订单平台。
- 券码/平台单号可复制。
...
21. **确认页消费顾客**：根态散客｜会员；会员未选禁用确认；加入会员手机号或称呼任一建档；无称呼→「尾号」+后四位；手机号 11 位；成功页仅摘要。
...
**确认页消费顾客 / 成功摘要**
22. **Given** 确认页根态（未展开）且性别默认男，**When** 直接确认核销，**Then** 订单与成功页消费顾客为「男散客」。
