...
1. **产品名「团购核销」**：覆盖抖音与美团双平台**团购券**完整核销。**系统优惠券**支持核销，走线上 APP 既有优惠券能力（原型示意入口）。**代金券 / 计次卡**本期不支持，扫码识别后通俗提示。输码/明细 Tab 仍含优惠券示意 / 抖音 / 美团。
...
4. **双平台同屏链路**：抖音与美团共用同一屏幕链（扫码→输码→结果→确认→成功/失败→明细→详情→撤销→匹配/员工）；仅平台文案、种子订单与演示数据不同。**扫码页无平台 Tab**：扫码后**自动识别券类型**（抖音团购券 / 美团团购券 / 系统优惠券 / 代金券 / 计次卡）：团购券进验券主流程；系统优惠券进**线上能力示意页**；代金券/计次卡 → **不支持弹窗**。输码与明细 Tab 为 **优惠券（示意）/ 抖音 / 美团**。
5. **授权分平台、相互独立**：抖音与美团**均经汇付天下 ISV**（独立指引页 `owner-auth` / `owner-auth-mt`，各四步）；验券/核销按**当前平台**校验对应授权态。产品授权态为 **未开通 / 已授权 / 已到期**。
6. **业绩归属一律默认空**：核销确认页点击选择服务员工；**不选也可确认核销**，按「未选服务员工」生成订单，**该单不计业绩**。成功页「操作人」= 当前登录账号，**与业绩归属解耦**。未选时「未选」与箭头均为错误红 `#F32F41`；已选为信息蓝。
7. **团购券无事先价目映射**：验券后可无消费项（内部 `mismatched`）；核销时手工「选择消费项」（项目/产品分组多选）。同一会话内按券名记忆匹配。**结果/确认页不展示「已匹配 / 未匹配」标签**（匹配能力保留）。多项时展示文案「多项匹配按门店原价占比分摊计算业绩」（**不展示具体百分比**）。单匹配时「匹配项目」与项目名垂直居中对齐，「修改」同行居中；未选消费项时 CTA 亦垂直居中。
...
9. **内部开单字段**：UI 支付方式随平台显示「抖音」或「美团」（**对齐线上支付方式称呼**）；内部写入 `orderType=快捷开单`、`payType=团购`。
10. **优惠券 Tab**：系统优惠券核销沿用线上 APP 既有能力（原型示意）；团购核销主链路不做代金券/计次卡。
...
14. **演示条**：复用现有演示条；券类型先行（抖音团购券 / 美团团购券 / 系统优惠券 / **代金券** / **计次卡**），点击即模拟扫码并**自动识别**；系统优惠券 → 线上能力示意页；代金券/计次卡 → 不支持弹窗；扫码结果场景（识别失败 / 已用过 / 价目未匹配 / 价目已匹配 / 多匹配）在**团购券**类型下演示（非团购类型时匹配相关场景置灰）。
...
18. **接入身份（v1.5.0）**：**抖音/美团均经汇付天下 ISV**（非抖音直连；非另接美团开放平台双通道）。门店绑定主路径：`external_id` / `out_request_no` ↔ `store_id`（失败再人工补录 / **门店绑定码**）。ID 分 L1 目录 / L2 订单 / L3 券实例。L1 目录能力**待汇付确认**；原型双平台均有 seed 目录示意。本期不做汇付分账抽佣。详见**模块 13**。
19. **授权态（v1.4）**：产品与原型演示条均为 **未开通 / 已授权 / 已到期**（已去掉「待确认」）。
20. **店长闭环原型（v1.4.2）**：双平台映射演示条；授权回跳模拟；**绑定码 + 已关联**映射（K1）；映射门禁弹窗；结果/确认/详情展示券码+平台单号（**不展示**团单/商品 ID）；无权限「查看授权指引」按当前平台跳转。
21. **团购设置（v1.5.1）**：工作台入口 → 双平台卡片（已授权显示「共 N 项」；品牌方标用 Figma SVG `douyin-logo.svg` / `meituan-logo.svg`）→（已授权且已映射）L1 团购券列表（双平台阴影券卡 + 分割线）→ 默认配置顶卡（24 方标 +「团购券」+ 名称 + 面额）**仅匹配项目（含每项数量）**，无默认业绩归属、**无页底「保存后预填」提示**；核销仍预填默认匹配，确认页仍可改业绩归属。
22. **品牌方标 vs 订单标签（v1.5.1）**：全站抖音/美团**品牌方标**统一为上述 Figma SVG；核销明细等处的**黄底白字「美团」标签**仍用 `meituan-tag.svg`（与方标分离）。
23. **匹配标签移除（v1.5.3）**：验券结果 / 确认核销页**不再展示**「已匹配 / 未匹配」标签；选择消费项、会话记忆、未选也可核销等能力不变；演示条「价目已匹配 / 价目未匹配」保留。
24. **券类型范围（v1.5.3）**：本期团购核销主链路仅**团购套餐券**；**代金券 / 计次卡**识别后弹窗通俗说明不支持；**系统优惠券**支持核销并沿用线上 APP 既有能力。
25. **筛选无重置（v1.5.3）**：核销明细 `filterMask` **无「重置」**，仅取消 / 确定。
26. **1C/1D/1E 返回层级（v1.5.4）**：1C→工作台、1D→1C、1E→1D；工作台宫格 / 左侧演示导航 / `?flow=` 深链进入均一致。
27. **支付方式称呼（v1.5.5）**：确认核销页「支付方式」随平台显示 **「抖音」/「美团」**（对齐线上）；**平台标识**仍为「抖音团购」/「美团团购」。
28. **1C 卡片标题（v1.5.6）**：团购设置双平台卡片标题改为「抖音团购」/「美团团购」（原「抖音核销」/「美团核销」）；**授权相关页仍称「抖音核销授权」**。
...
- 产品更名为 **「团购核销」**，抖音与美团双平台**团购券**完整核销；**系统优惠券**走线上 APP 既有能力；**代金券 / 计次卡**提示不支持；
- 本期按 **单店** 交付，不做总部/多分店与「业绩归属配置」页；
- 团购券与门店价目 **无事先映射**，需在核销时手工选择消费项，并支持会话记忆；**不展示匹配状态标签**；
- 业绩归属与操作人解耦：归属可空（不计业绩），操作人固定为登录账号；
- 抖音与美团授权**相互独立**；**双平台均经汇付天下 ISV**（独立四步指引页）；
- **扫码页去平台 Tab**，扫码后自动识别券类型（团购券 / 系统优惠券 / 代金券 / 计次卡）；美团流程页面头部标识不再出现抖音 logo，随平台动态切换；
...
| 范围可控 | 系统券走线上能力示意；代金券/计次卡不支持；单店；无业绩归属配置页；目录待汇付确认 |

## 1.3 本期做（In）

- 演示占位首页：顶部扫码入口进入核销主链路（**非真实 APP 首页**）
- 扫码核销（自动识别：团购券 / 系统优惠券示意 / 代金券·计次卡不支持）、输码验券（优惠券示意 / 抖音可用 / 美团可用）
...
- 核销明细（优惠券示意 + 抖音列表 + 美团列表 + 筛选；**列表不展示 refund**；筛选无「已退款」、**无重置**）→ 订单详情 → 撤销（1h）→ 撤销成功
- 抖音授权指引（汇付四步，汇付 ISV）；美团授权指引（汇付四步，独立页）
...
- 总部 / 多分店切换、「业绩归属配置」页
- 团购核销主链路内重做系统优惠券完整 UI（沿用线上 APP）；代金券 / 计次卡核销
- 美团授权正式指引页菜单级截图细路径（本期双平台四步示意已落地；汇付后台精确菜单【待商务材料】）
- 次卡 / 其它平台券（非抖音/美团团购）
...
| 美团同页核销 | 美团完整核销（与抖音同链）；系统券走线上能力；代金券/计次卡不支持 |
...
| 未授权 / 到期（抖音） | 引导完成汇付授权 | 动作拦截 → `owner-auth`；状态展示正确 |
...
团购核销（单店 · 抖音/美团）
 ...
 ├─ owner-auth 抖音授权指引（汇付）    <!-- #owner-auth -->
...
| 撤销成功 | `revoke-ok` | 撤销结果 | |
| 抖音授权（汇付） | `owner-auth` | 抖音授权 | 汇付四步指引 + 状态（接入通道汇付） |
| 美团授权（汇付） | `owner-auth-mt` | 美团授权 | 汇付四步指引 + 状态 |
...
| 值 | 含义 | UI 文案要点 |
|----|------|-------------|
| `douyin` | 抖音团购 | 「抖音」「抖音团购」；授权走 `auth` → `owner-auth`（汇付） |
| `meituan` | 美团团购 | 「美团」「美团团购」；授权走 `authMeituan` → `owner-auth-mt`（汇付） |
| （优惠券 Tab） | 示意 | 沿用线上 APP 系统优惠券核销；不在本原型展开完整 UI |

扫码页无平台 Tab：扫码后**自动识别券类型**并更新 `platform`（系统券→示意页不改 `platform`；代金券/计次卡→不支持弹窗）；输码页、明细页切换平台 Tab 亦更新 `platform`。结果/确认/成功等页文案、头部标识与授权门禁随 `platform` 切换。
...
### 4.3.2 美团授权 `authMeituan`（独立于抖音；经汇付）
...
- **美团授权与抖音授权相互独立**；一侧 `ok` 不影响另一侧。
- `owner-auth` / `owner-auth-mt` 均为汇付 ISV 四步正式指引（菜单细路径【待汇付材料】）。
...
原型左侧演示条仅供评审，**不算产品正式规则**。**扫码页顶部无平台 Tab**；演示条第一行选**券类型**（抖音团购券 / 美团团购券 / 系统优惠券 / 代金券 / 计次卡），**点击即模拟扫码自动识别**：抖音 / 美团团购券进入对应平台验券结果；**系统优惠券**进入线上能力示意页；**代金券 / 计次卡**弹出不支持说明。第二行在所选券类型下演示结果场景：
...
- 「团购核销」右侧两行热区：
  - 抖音：logo +「抖音」+ 状态文案 + › → `owner-auth`（汇付指引）
...
- 扫码后**自动识别券类型**：抖音团购券 → 抖音验券结果；美团团购券 → 美团验券结果（结果/确认等页头部标识随平台切换，美团不再出现抖音 logo）；**系统优惠券 → 线上 APP 核销示意页**；**代金券 / 计次卡 → `unsupportedCouponMask` 通俗不支持提示**。
...
- 「取消」/ 返回不写入；「确定」写入会话 `selectedMatchIds`（及数量）+ `matchMemory[券名]`，回 `confirm`（**不改匹配状态标签**，因已无标签）。
- 自团购设置进入时：写入 `dealDefaults[L1].matches: [{ id, qty }]`（仅匹配，无业绩）。
...
### 6.13.1 抖音授权（汇付天下 ISV） <!-- #owner-auth -->

- 标题「抖音授权」（与美团同为汇付四步结构，**独立页面**）。
- 四步（对齐汇付）：确认抖音来客/商家门店 → 打开汇付天下商户/ISV 后台 → 开通并授权剑琅联盟（系统带门店标识）→ 回剑琅确认状态。
- 「去入驻」「打开汇付说明」「我已完成授权，刷新状态」；`dy-auth-grant` 保留**待审核**态示意。
- 状态卡：授权态、到期日、**接入通道：汇付天下 ISV**、「已关联平台门店」是/否、可复制**门店绑定码**（K1 保留）。
- 提示：抖音团购核销**经汇付透传**；验券凭顾客券码；本期不含分账抽佣。
- **仅服务抖音平台动作拦截**；与美团授权无关。

### 6.13.2 美团授权（汇付天下 ISV） <!-- #owner-auth-mt -->

- 标题「美团授权」。
- 四步：确认美团商家门店 → 打开汇付天下商户/ISV 后台 → 开通并授权剑琅联盟（系统带门店标识）→ 回剑琅确认状态。
- 「去入驻」「打开汇付说明」「我已完成授权，刷新状态」；状态卡含**接入通道：汇付天下 ISV**、已关联是/否、绑定码。
- 提示：美团团购核销**经汇付透传**；验券凭顾客券码；本期不含分账抽佣。
- 抖音侧动作**不**进入本页；本页仅承接美团验券/核销门禁。

### 6.13.3 映射门禁（模块 13）

- 验券/核销动作：`rolePerm` → 平台授权三态 → **门店映射已绑定**。
- 授权非 ok：弹窗区分未开通「去开通」/ 已到期「去续期」。
- 授权 ok 但未映射：弹窗「请完善门店绑定」并展示绑定码 → 进对应授权指引。
- 左侧演示条可独立切换双平台「已绑定 / 未绑定」。
- 无核销权限弹窗「查看授权指引」按**当前平台**跳转。
...
**标题栏返回层级（v1.5.4）**
- `tg-set`（1C）标题栏返回 → **工作台**；`tg-deals`（1D）→ **团购设置（1C）**；`tg-config`（1E）→ **团购券列表（1D）**。
- 该层级与**进入路径无关**：工作台宫格、左侧演示导航、`?flow=` 深链进入均一致；进入页面时历史栈按该层级初始化，故逐级返回可用。

**首页（双卡片 · `tg-set`）**
- 左右边距 16；纵向两大卡片：「抖音团购」「美团团购」（v1.5.6 起，原「抖音核销 / 美团核销」）。
- 视觉：白底、圆角 14、阴影 `1px 2px 5px rgba(0,0,0,0.08)`（无描边）；左侧 **48×48** 平台方标（Figma SVG：`assets/icons/douyin-logo.svg` / `meituan-logo.svg`）；右侧灰色圆底右箭头（`chevron-circle-right.svg`）。
- 状态文案：未开通「去开通」；已到期「授权已到期，去续期」；其它非 ok 态 tip 为橙色；**已授权**展示「共 N 项」（目录条数，灰色 `#929292`，非 tip）。
- 点击：非 ok → 对应授权指引（`owner-auth` / `owner-auth-mt`）；ok 但映射未绑定 → 绑定码弹窗；ok 且已绑定 → 团购券列表。

**团购券列表（`tg-deals`，L1 目录）**
- 数据：接口拉取目录能力**待汇付确认**；**原型双平台均有 seed 目录**示意。
- 双平台用户端统一称「团购券」：标题「抖音团购券」/「美团团购券」（**不用「美团团单」**）。
- 列表均为**阴影券卡**（Figma 对齐）：白底圆角 14 + 同款阴影；主体区与右侧 16px chevron（`chevron-right-16.svg`）横排。
  - 首行：24 方标 + 券名（17 Bold）+ 金额（`#FE2C55` 靠右）。
  - 分割线后次行：与**券名左对齐**（`padding-left: 32`，不对齐 logo）；胶囊「已配置」（绿底 `rgba(58,184,131,0.14)` / 字 `#1F8A5B`）或「未配置」（灰）；已配置摘要 = **全部匹配项目名**（数量>1 时名后接 `×N`，可换行），**不含员工/业绩归属**。
- 点击一项 → 默认配置页。

**默认配置（`tg-config`）**
- 顶部券卡（平台分态）：24 方标 + 文案「团购券」→ 券名 → 分割线 → 「面额」/ ¥金额；**无**页底「保存后…预填…」提示条。
- 可配：**仅匹配项目**（每项带数量步进器 1–99）；**无默认业绩归属**。
- 匹配区 tip（info）：「多项匹配按门店原价占比分摊计算业绩」。
- 存储：按 L1 ID 写入 `dealDefaults`，结构 `matches: [{ id, qty }]`。
- 底部「取消」「保存」。
- 核销预填优先级：**团购设置默认匹配 > 会话记忆 > 空**；确认页仍可改匹配；**业绩归属仅在确认页手工选择**（设置页不配）。

**用户端不展示「团单/商品 ID」**（结果/确认/成功/详情隐藏；授权目录示意可保留 id）。
...
| `unsupportedCouponMask` | 不支持券类型 | 代金券/计次卡通俗说明；单按钮「我知道了」 |
...
| `dealDefaults` | `{ [L1Id]: { matches: [{ id, qty }] } }` 团购设置默认匹配（含数量 1–99） |
...
| 门店映射绑定 | 回调自动绑定 / 人工补录 / 绑定码（K1） | 见模块 13.3【待联调确认】 |
...
1. **产品名「团购核销」**：抖音 + 美团团购券完整核销；系统优惠券走线上能力；代金券/计次卡不支持。
...
6. **授权独立**：抖音与美团均经汇付（`auth`/`owner-auth` 与 `authMeituan`/`owner-auth-mt`）互不影响。
...
10. **确认页布局**：单匹配时「匹配项目」与项目名、修改垂直居中；未选消费项 CTA 垂直居中。
...
23. **团购设置**：双平台阴影卡片 + Figma 品牌方标；已授权「共 N 项」；列表阴影券卡（分割线 + 已配置/未配置）；默认配置顶卡无页底预填提示；仅匹配项目+数量；`dealDefaults.matches:[{id,qty}]`。
24. **品牌资源**：方标 `douyin-logo.svg` / `meituan-logo.svg`；订单黄底「美团」标签仍为 `meituan-tag.svg`。
25. **范围边界**：其它收银/会员能力沿用线上；冲突以原型界面修正 PRD。
---
...
**导航/按钮**：扫码、验券订单、输入券号、立即验券、重新扫码、去核销、选择消费项、修改、取消、确认核销、确定、清空、继续核销、完成、筛选、应用、撤销核销、返回核销明细、查看原单、手动输码、查看订单、查看授权指引、重试核销、我知道了、项目、产品、优惠券、抖音、美团、点客、散客、大工、中工、小工、男、女。

**标签/状态**：已核销、已撤销、已退款（**仅详情等非列表场景**）、未开通、已授权、已到期、抖音、美团、抖音团购、美团团购。（**已去除**验券流「未匹配 / 已匹配」标签）

**关键提示**：将券码放入框内，扫码后自动识别券类型；多项匹配按门店原价占比分摊计算业绩；未匹配价目时，将按券面额开单；业绩归属未选则该单不计业绩；暂不支持核销代金券/计次卡（当前只能核销团购套餐券，请换一张团购券再试）；已自动生成开单，可在验券订单中查看明细；对应开单已冲销，员工业绩已回滚，撤销流水已保留；经汇付完成门店授权后，方可在本店核销对应平台团购券；当前账号未开启「收银开单」权限，无法扫码核销…
...
| 抖音门店授权 `auth` | 仅 `ok` 可验券/核销（平台=抖音）；否则引导 `owner-auth`（汇付） |
| 美团门店授权 `authMeituan` | 仅 `ok` 可验券/核销（平台=美团）；否则引导 `owner-auth-mt`（汇付）；**独立于** `auth` |
...
9. **Given** 确认页仅 1 个匹配项，**When** 查看匹配行，**Then**「匹配项目」与项目名垂直居中，「修改」同行居中。
...
18b. **Given** 扫码识别为代金券或计次卡，**When** 识别完成，**Then** 弹出通俗不支持提示（如「暂不支持核销代金券/计次卡。当前只能核销团购套餐券，请换一张团购券再试。」），不进入核销。
...
29. **Given** 演示条点「代金券」或「计次卡」，**When** 模拟扫码，**Then** 弹出不支持提示——**仅演示，非产品规则**。
...
- 本期仅单店；双平台团购券（抖音/美团）+ 系统券线上能力示意；代金券/计次卡不支持。
- 交付只需 PRD + 交互原型。
...
> 原型左侧「演示条」（网络/相机/**收银开单权限**切换、券类型：抖音团购券 / 美团团购券 / 系统优惠券 / 代金券 / 计次卡——点击即模拟扫码自动识别；扫码结果演示：识别失败 / 已用过 / 价目未匹配 / **价目已匹配** / 多匹配；**无**「扫码成功」；含美团演示订单与系统券示意）仅供评审演示，**不作为产品正式规格**；正式规则以本文状态机与业务规则为准。
...
| 主题 | 结论 |
|------|------|
| 产出 | PRD 文字（含流程图）+ **技术联调清单**；双平台授权指引文案同步（汇付四步） |
| SDK | **不做**抖音/美团内嵌官方 SDK；授权在汇付商户/ISV 后台完成；剑琅 APP 仅引导页 + 后端 API |
| 接入通道 | **抖音/美团均经汇付天下 ISV**（非抖音直连开放平台；非另接美团开放平台双通道） |
| 抖音身份 | 我方 = 汇付天下 ISV 下游；商户在汇付后台开通/授权「剑琅联盟」完成抖音团购核销能力 |
| 美团身份 | 同通道：汇付侧**透传**美团团购验券/核销/（待确认的）目录能力 |
| 汇付边界 | 本期**不做**汇付分账/抽佣产品设计；双平台团购核销业务数据经汇付通道约定落地 |
| 权限生效 | **汇付授权成功即「已授权」**；我方只同步状态，**不做二次审核**（抖音 `dy-auth-grant` 原型可示意待审核） |
| 门店绑定主路径 | 汇付开通/进件带 **`external_id` / `out_request_no` ↔ `store_id`**。失败再 **人工补录** 或 **门店专属绑定码**（K1：绑定码 + 已关联映射保留） |
| 组织 | 严格单店：1 APP 门店 ↔ ≤1 抖音门店 + ≤1 美团（汇付）门店 |
| 三层 ID | 见 13.3.0：`product_id/deal_id`（目录） / `order_id`（订单） / `coupon_code`·券实例（验券） |
| 团购券目录 | **双平台** L1 预拉能力 **均待与汇付确认**；验券主路径不依赖预拉；**原型双平台均有 seed 目录** |
| 关联目的 | 后台对账/运营 + 为将来「券↔价目事先映射」预留；**本期核销仍手工匹配消费项**（设置可预填匹配+数量） |
| 关联粒度 | **门店级**；不做团购券强制绑价目 |
| 闭环叙事 | 双平台：引导→（可入驻）→汇付开通/授权（带 external_id）→回调映射→（可选拉目录）→经汇付码核销 |
| 入驻 | 未入驻时 APP「去入驻」外链（不代注册） |
| 验券凭证 | 顾客券码 / 二维码 / 短链 |
| 授权产品态 | **未开通 / 已授权 / 已到期** |
| 图示 | **业务总览图** + **研发映射图** |

> **与外部「汇付=仅支付」通识的对齐**：行业上汇付常被描述为资金通道；**本产品路径已确认抖音与美团团购核销均走汇付透传**，联调以汇付团购相关 API/文档为准，不以「仅支付流水」假设设计验券。
...
2. 未开通：按指引进入**汇付天下商户/ISV 后台**完成对剑琅的开通/授权（抖音团购核销**经汇付透传**，非 APP 内嵌 SDK）。  
3. 无来客/商家门店时先「去入驻」，再回汇付。  
4. 开通/进件时系统写入 `external_id`/`out_request_no`=当前 `store_id`，回调后自动映射；失败则人工/绑定码。  
...
1. APP「团购核销 → 美团」看状态。  
2. 未开通：指引进入**汇付天下商户/ISV 后台**完成对剑琅的开通/授权（美团团购核销**经汇付透传**）。  
3. 无美团门店时先「去入驻」，再回汇付。  
4. 开通/进件时系统写入 `external_id`/`out_request_no`=当前 `store_id`，回调后自动映射；失败则人工/绑定码。  
5. 团购券目录预拉：以汇付能力清单为准；未确认前验券主路径不依赖目录。  
6. 同屏核销；支付方式 UI「美团」。
...
**绑定优先级（主路径对齐汇付进件惯例；K1 绑定码 + 已关联保留）**

1. **汇付开通/进件**（抖音与美团同通道）：请求携带 `external_id` 或 `out_request_no` = `app_store_id`（或可逆编码），回调/查询还原后绑定。  
2. 上述失败 → **人工补录**平台门店 ID / 汇付商户号。  
3. 或 APP 展示**门店专属绑定码**，商户在汇付流程中填入后校验绑定；状态卡展示「已关联」是/否。  
4. 同一 `plat` 下禁止两 APP 门店抢同一 `plat_store_id`；已绑定重复授权视为续期/刷新。
...
| J6 | 「去入驻」外链 | 【待产品补链接】 | |
| J7 | **本期不做**汇付分账/抽佣 | Out | |
| J8 | `dealDefaults.matches:[{id,qty}]` 默认匹配 | 【待实现】 | 无默认业绩归属 |
...
**已由 v1.4.2–v1.5.6 原型拍板：** 映射缺失弹窗文案；绑定码 + 已关联（K1）；授权三态 UI；用户端不展示团单/商品 ID；团购设置默认**仅匹配项目+数量**（按 L1）；美团用户端称「团购券」；双平台 seed 目录；**v1.5.1** 品牌方标 SVG、1C/1D/1E 阴影券卡视觉、默认配置去页底预填提示；**v1.5.3** 去掉匹配状态标签、代金券/计次卡不支持提示、筛选无重置、系统券走线上能力示意；**v1.5.4** 1C/1D/1E 标题栏返回层级修复；**v1.5.5** 支付方式称呼对齐线上（抖音 / 美团）；**v1.5.6** 1C 卡片标题改「抖音团购 / 美团团购」。
