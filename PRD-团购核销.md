...
18. **接入身份（v1.4.1）**：抖音=生活服务技术服务商直连；美团团购核销**仅经汇付透传**（非另接美团开放平台双通道）。门店绑定主路径：`state`（抖音）/ `external_id`（汇付）↔`store_id`。ID 分 L1 目录 / L2 订单 / L3 券实例。抖音拉 L1 须资质+授权范围。本期不做汇付分账抽佣。详见**模块 13**。
19. **授权态（v1.4）**：产品与原型演示条均为 **未开通 / 已授权 / 已到期**（已去掉「待确认」）。
20. **店长闭环原型（v1.4.2）**：双平台映射演示条；授权回跳模拟；绑定码；映射门禁弹窗；结果/确认/成功/详情展示券码+平台单号+团单/商品 ID；抖音团单列表示意 / 美团目录占位；无权限「查看授权指引」按当前平台跳转。
...
- 标题「抖音来客授权」。
- 四步：打开来客 → 服务市场/授权管理 → 勾选核销相关能力 → 回剑琅确认状态（自动关联门店；目录同步有权限前提）。
- 「去入驻」「我已完成授权，刷新状态」示意；状态卡含「已关联平台门店」是/否、可复制**门店绑定码**；授权且已绑定时展示团单目录示意列表。
- 提示：须为生活服务服务商且授权覆盖核销相关能力；验券凭顾客券码。
- **仅服务抖音平台动作拦截**；与美团授权无关。
...
- 「去入驻」「打开汇付说明」「我已完成授权，刷新状态」；状态卡含接入通道、已关联是/否、绑定码；团单目录为「目录未开通（待汇付确认）」占位。
...
> 读者：前端 ✓ · 后端 **必看** · 测试 ✓  
> 依据：2026-09-09 产品确认（Q1–Q15）；2026-09-09 DeepSeek 分享核对后修订（Q1A/Q3A/Q4A/Q5A/Q6A/Q7B）

## 13.0 确认结论摘要

| 主题 | 结论 |
|------|------|
| 产出 | PRD 文字（含流程图）+ **技术联调清单**；美团授权指引文案同步 |
| SDK | **不做**抖音/美团内嵌官方 SDK；授权在平台 App/网页或汇付后台完成；剑琅 APP 仅引导页 + 后端 API |
| 抖音身份 | 我方 = 抖音来客**技术服务商（生活服务 ISV）**；商户在「来客 → 服务市场」找「剑琅联盟」发起门店授权 |
| 美团身份 | **仅经汇付天下 ISV**：产品确认汇付侧**透传**美团团购验券/核销/（待确认的）团单能力；**不是**「汇付只做收款、另接美团开放平台」双通道 |
| 汇付边界 | 本期**不做**汇付分账/抽佣产品设计；团购核销业务数据按上条经汇付通道约定落地 |
| 权限生效 | **平台/汇付授权成功即「已授权」**；我方只同步状态，**不做二次审核** |
| 门店绑定主路径 | 抖音/美团：授权链路带 **`state` ↔ `store_id`**；汇付：进件/开通带 **`external_id` / `out_request_no` ↔ `store_id`**。失败再 **人工补录** 或 **门店专属绑定码** |
| 组织 | 严格单店：1 APP 门店 ↔ ≤1 抖音门店 + ≤1 美团（汇付）门店 |
| 三层 ID | 见 13.3.0：`product_id/deal_id`（目录） / `order_id`（订单） / `coupon_code`·券实例（验券） |
| 团单目录 | **抖音**：在资质+授权范围满足时**可拉**；**美团**：经汇付是否可预拉团单列表 **仍待与汇付确认能力清单**（验券主路径不依赖预拉） |
| 关联目的 | 后台对账/运营 + 为将来「券↔价目事先映射」预留；**本期核销仍手工匹配消费项** |
| 关联粒度 | **门店级**；不做团单强制绑价目 |
| 闭环叙事 | 抖音：引导→（可入驻）→来客找应用→授权（带 state）→同步映射→拉目录→码核销；美团：引导→汇付开通/授权（带 external_id）→回调映射→（可选拉目录）→经汇付码核销 |
| 入驻 | 未入驻时 APP「去入驻」外链（不代注册） |
| 验券凭证 | 顾客券码 / 二维码 / 短链 |
| 授权产品态 | **未开通 / 已授权 / 已到期** |
| 图示 | **业务总览图** + **研发映射图** |

> **与外部「汇付=仅支付」通识的对齐**：行业上汇付常被描述为资金通道；**本产品路径已确认美团团购核销走汇付透传**，联调以汇付团购相关 API/文档为准，不以「仅支付流水」假设设计验券。
...
flowchart TB
  ...
  subgraph DY["抖音（直连生活服务 ISV）"]
    Laike[抖音来客]
    Mkt[服务市场·剑琅联盟]
    DyAPI[抖音开放能力·门店/商品/核销]
  end

  subgraph MT["美团（仅经汇付透传）"]
    HuiFu[汇付商户/ISV·开通授权]
    MtCap[美团团购验券核销·经汇付]
  end

  Guide -->|state↔store_id| Laike
  Laike --> Mkt
  Mkt -->|授权回调+state| StoreMap
  StoreMap --> DyAPI
  DyAPI -->|拉 product_id 目录·有前提| DealCat
  DyAPI -->|券码验券| Verify

  Guide -->|external_id↔store_id| HuiFu
  HuiFu -->|回调/查询| StoreMap
  StoreMap --> MtCap
  MtCap -.->|deal_id 目录·待汇付确认| DealCat
  MtCap -->|券码验券核销| Verify
```

| 平台 | 我方角色 | 商户完成授权的主场所 | 业务数据来源 |
|------|----------|----------------------|--------------|
| 抖音 | 生活服务技术服务商（直连） | 抖音来客 · 服务市场 | 抖音开放接口（直连） |
| 美团 | 汇付天下 ISV 下游 | 汇付商户 / ISV 后台 | **经汇付透传**美团团购能力 |

**本期 Out：** 汇付分账/技术服务费抽佣产品；另接美团到店/餐饮开放平台双通道（与 Q1A 冲突，不做）。
...
flowchart TD
  ...
  S5 --> S6[服务市场搜索「剑琅联盟」]
  S6 --> S7[发起门店授权·带 state↔store_id]
  S7 --> S8[平台授权生效]
  S8 --> B1[剑琅后端：回调还原 state→store_id]
  B1 --> B2{映射写入}
  B2 -->|成功| B3[写入映射表]
  B2 -->|失败| B4[人工补录 或 绑定码]
  B4 --> B3
  B3 --> B5{资质与授权范围含商品同步?}
  B5 -->|是| B6[拉取 product_id/goods_id 目录]
  B5 -->|否| B7[跳过目录·仍可用券码验券]
  B6 --> S9
  B7 --> S9
  S9 --> S10[顾客出示券码/码/短链]
  S10 --> S11[扫码或输码验券·落 order_id/券实例]
...
3. 在服务市场找到**剑琅联盟**，发起授权并勾选门店信息 / 团购核销 / 对账等最小集（后端携带 `state`↔当前 `store_id`）。  
4. 平台授权成功后，回 APP 刷新为**已授权**（我方同步，非人工审）。  
5. 系统用回调中的 `state` 自动绑定 APP 门店；失败则人工补录或绑定码。  
6. **仅当**公司具备生活服务服务商资质、且本店授权范围含商品/团购相关能力时，后台拉取团单目录；否则跳过目录，仍可靠券码验券。  
7. 店员扫码/输码验券 → 确认核销 → 开单。
...
1. APP「团购核销 → 美团」看状态。  
2. 未开通：指引进入**汇付天下商户/ISV 后台**完成对剑琅的开通/授权（美团团购核销**经汇付透传**，非 APP 内嵌 SDK）。  
3. 无美团门店时先「去入驻」，再回汇付。  
4. 开通/进件时系统写入 `external_id`/`out_request_no`=当前 `store_id`，回调后自动映射；失败则人工/绑定码。  
5. 团单目录预拉：以汇付能力清单为准；未确认前验券主路径不依赖目录。  
6. 同屏核销；支付方式 UI「美团团购」。
...
|------|------|------------------------------|----------|------|
| L1 目录 | 商家创建的团购商品/团单 | 抖音 `product_id` / `goods_id`；美团 `deal_id` 等 | 授权后调「商品/团单列表」（有前提） | 对账、运营、将来价目映射预留 |
...
- 美团侧 L1/L2/L3 均经**汇付约定接口**获取（产品路径 Q1A）。
...
flowchart LR
  ...
  subgraph Catalog["L1 目录层"]
    DDeal[抖音 product_id 列表·有前提]
    MDeal[美团 deal_id 列表·待汇付确认]
    DY --> DDeal
    MT -.-> MDeal
  end
...
| `huifu_merchant_id` | 美团通道：汇付商户号/子商户号 |
| `auth_status` | `none` \| `ok` \| `expired` |
| `auth_expire_at` | 到期时间 |
| `bind_source` | `state` \| `external_id` \| `manual` \| `bind_code` |
| `updated_at` | 最后同步时间 |

**绑定优先级（主路径对齐业界 OAuth/进件惯例）**

1. **抖音 /（若走 OAuth 的）美团业务授权**：发起授权前生成 `state`，Redis/DB 映射 `{ state → app_store_id }`，回调原样带回后绑定 `plat_store_id`。  
2. **汇付开通/进件**：请求携带 `external_id` 或 `out_request_no` = `app_store_id`（或可逆编码），回调/查询还原后绑定。  
3. 上述失败 → **人工补录**平台门店 ID / 汇付商户号。  
4. 或 APP 展示**门店专属绑定码**，商户在来客/汇付流程中填入后校验绑定。  
5. 同一 `plat` 下禁止两 APP 门店抢同一 `plat_store_id`；已绑定重复授权视为续期/刷新。

### 13.3.2 团单/商品目录（L1）

| 平台 | 能否拉 L1 | 前提 | 本期核销是否依赖 L1 |
|------|-----------|------|---------------------|
| 抖音 | **可以**（有前提） | ① 公司为抖音生活服务服务商；② 本店已授权；③ 授权范围含商品/团购同步类能力 | **不依赖**；无 L1 仍可用 L3 码验券 |
| 美团 | **待汇付确认**其团单列表 API | 汇付侧已开通且映射完成 | **不依赖**；验券响应落 L2/L3 即可 |
...
```
店长(APP) → 打开来客指引
剑琅后端 → 生成 state↔store_id →（服务市场授权 URL 或指引上下文）
店长(来客) → 服务市场授权剑琅联盟
抖音 → 回调 auth_code + state → 剑琅后端
剑琅后端 → 还原 store_id → upsert 门店映射(auth=ok)
剑琅后端 → 若资质+授权范围满足 → 拉 L1 商品列表 → upsert 目录
店长(APP) → 刷新=已授权
店员 → 扫码/输码 → 验券(券码+storeId) → 落 L2/L3 → 核销开单
```
...
店长(汇付) → 完成对剑琅开通/授权
汇付 → 回调/查询 → 剑琅后端还原 store_id → upsert 映射(auth=ok)
剑琅后端 → 【待汇付确认】拉 L1 deal 列表？→ 可则落目录 / 否则跳过
...
| # | 项 | 状态 | 备注 |
|---|----|------|------|
| D1 | 生活服务服务商资质 + 服务市场「剑琅联盟」能力最小集 | 【待联调确认】 | 门店/核销/对账；拉 L1 另需商品同步类权限 |
| D2 | 授权成功/解除/到期回调或主动查询 | 【待联调确认】 | 驱动三态 |
| D3 | **`state` 参数**往返与门店 ID 字段 | 【待联调确认】 | **绑定主路径** |
| D4 | 绑定码降级 | 【待联调确认】 | |
| D5 | L1 商品/团单列表（`product_id`/`goods_id`） | 【待联调确认】 | **有前提才拉**；无权限则跳过 |
| D6 | 验券/核销/撤销/幂等（输入券码/短链） | 【待联调确认】 | |
| D7 | 验券响应：L3 券实例、L1 ID、L2 `order_id`、适用门店 | 【待联调确认】 | |
| D8 | Token 刷新与到期 | 【待联调确认】 | |

### 13.6.2 美团（汇付天下 ISV · 团购透传）

| # | 项 | 状态 | 备注 |
|---|----|------|------|
| H1 | 汇付开通/授权商户操作文档与菜单 | 【待商务/联调】 | 指引细文案 |
| H2 | 回调字段：商户号、美团门店 ID 等 | 【待联调确认】 | |
| H3 | **`external_id` / `out_request_no` = store_id** | 【待联调确认】 | **绑定主路径** |
| H4 | **L1 团单列表 API 是否存在** | **【待与汇付确认】** | 不影响码核销主路径 |
| H5 | 验券/核销/撤销 API（经汇付透传美团团购） | 【待联调确认】 | 产品确认非「仅支付」 |
...
| J3 | L1 同步任务（抖音有前提；美团视 H4） | 【待实现】 | 不阻塞核销 |
...
1. 汇付侧 **L1 团单列表**是否提供（H4）  
2. 汇付后台菜单截图（细化指引第 2–3 步）  
3. 抖音服务市场授权是否完整支持 OAuth 式 `state`（若仅服务市场无 state，则用「APP 已登录门店会话 + 回调门店 ID」或绑定码兜底，联调裁定）

**已由 v1.4.2 原型拍板：** 映射缺失弹窗文案；绑定码在双平台授权状态卡 + `mapNeedMask`；授权三态 UI；结果/详情三层 ID 字段。
