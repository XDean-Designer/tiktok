...
12b. **流水消费项名（v1.6.3）**：门店流水（1a+）/ 核销明细**列表**主标题显示**核销团购券名**（非「抖音团购/美团团购」）；标题左侧「团」标区分团购单；48×48 仍为平台方标。**详情**已匹配→项目/产品名（可多项），未匹配→「快捷开单」。全局文案「快速消费」已更名为「快捷开单」。
...
16. **确认页消费顾客（v1.6.2）**：确认核销页新增「消费顾客」卡。默认「散客」；会员路径进入仅真实会员列表（无散客快捷入口），未选会员时「确认核销」按钮灰掉；散客二级分段「加入会员 | 不加入」默认不加入，不加入仅选性别（默认男→「男散客」）；加入会员时手机号或称呼任一有值即可建档（称呼空→「团购顾客0N」会话递增），手机号有值须 11 位否则 toast。草稿在会员/散客切换时保留。成功页**删除**原「选填」表单，仅展示一行顾客摘要。
...
38. **选择顾客 UI + 确认页顾客卡（v1.6.4）**：选择顾客页对齐 Figma 58:111（搜索+取消、字母索引、VIP/最近消费）；确认页一级/二级分段分层；点卡片自动滚动；性别马卡龙蓝/粉居中。
...
- 核销明细（优惠券示意 + 抖音列表 + 美团列表 + 筛选；**列表主标题为团购券名**；**列表不展示 refund**；筛选无「已退款」、**无重置**）→ 订单详情（匹配项或快捷开单 + 消费顾客）→ 撤销（1h）→ 撤销成功
...
| 确认页消费顾客 | 会员/散客分段；散客可加入会员或不加入 | 默认散客；会员未选则确认按钮禁用；加入会员：手机号或称呼任一即可建档；称呼空→团购顾客0N；手机号须 11 位 |
| 流水消费项名 | 列表显示核销团购券名；详情匹配项或快捷开单 | 1a+/核销明细列表均用券名；「团」标区分 |
...
确认核销页「消费顾客」卡；与员工业绩「点客/散客」**完全解耦**。成功页**无**选填表单（已删）。

| 规则 | 说明 |
|------|------|
| 默认 | 分段「会员 \| 散客」默认**散客** |
| 会员未选 | 「确认核销」按钮灰掉不可点 |
| 会员列表 `pick-member` | 仅真实会员（可含本次新建）；**不含**散客快捷入口；选中后回确认页摘要，可重新选择 |
| 散客 · 不加入（默认） | 仅选性别，默认「男」→ 订单记「男散客」/「女散客」 |
| 散客 · 加入会员 | 展开手机号 / 称呼 / 性别；**手机号或称呼任一有值**即可建档；称呼空 → 姓名「团购顾客0N」（会话内递增防重名）；手机号有值须 11 位，非法 → toast「请输入 11 位手机号」 |
| 加入会员但仅性别 | 不建档，仍记「男散客」/「女散客」 |
| 草稿 | 会员/散客切换时保留各自草稿 |
...
- **消费顾客**卡（v1.6.2）：见 **5.3**；会员 → `pick-member`。
...
- 一级「会员|散客」为大号描边胶囊（选中深底白字）；二级「加入会员|不加入」为小号灰底分段，以体现层级。
- 点卡片内控件时，内容区自动滚动使「消费顾客」卡完整可见。
- 性别控件居中；选中男马卡龙蓝（底 `#B8D4F0` / 字 `#3A6EA5`），女马卡龙粉（底 `#F5C6D0` / 字 `#C45A7A`）。
...
**首页卡片**：已开通「共 N 项」（N = 当前平台**已绑门店**目录 L1 **按店累加**合计，同券多店各计 1；演示绑 3 店时抖音/美团均为 4+3+2=**9**）；未开通「去开通」；已到期「授权已到期，去续期」。点击：非已开通 → `auth-open`；已开通 → `tg-deals`。
...
明细（列表标题=团购券名；过滤 refund）→ 详情（匹配名或快捷开单）→（1h 内）撤销 → revoke-ok
```
...
21. **确认页消费顾客**：默认散客；会员未选禁用确认；加入会员手机号或称呼任一建档；团购顾客0N；手机号 11 位；成功页仅摘要。
22. **流水命名**：列表显示核销团购券名；详情匹配项或「快捷开单」。
23. **1a+ 视觉**：Tab 五等分不横滑；统一顾客头像；消费项左「团」标。
22. **输码演示关键字**（used/fail/mismatch 等）及扫码结果演示条：**仅原型演示，不算产品规则**；演示尊重当前平台；含美团演示订单。
23. **团购设置**：双平台阴影卡片 + Figma 品牌方标；已开通「共 N 项」（已绑门店目录**按店累加**，非去重）；多店时 `tg-deals` 顶 TAB（同输码验券样式、全名智能换行、3 项不横滑）；列表阴影券卡（分割线 + 已配置/未配置）；默认配置顶卡无页底预填提示；仅核销项目+数量；`dealDefaults[storeId][L1].matches:[{id,qty}]`；核销预填取登录门店桶。开通页「去设置团购券」直达 `tg-deals`，返回开通页。
...
**确认页消费顾客**：会员 / 散客；加入会员 / 不加入；手机号、称呼、性别。

**成功页字段**：消费顾客摘要一行；无选填表单。

**流水命名**：列表主标题为核销团购券名；详情匹配项或「快捷开单」。1a+ 消费项左「团」标。
...
**确认页消费顾客 / 成功摘要**
22. **Given** 确认页默认散客且不加入、性别默认男，**When** 直接确认核销，**Then** 订单与成功页消费顾客为「男散客」。
23. **Given** 选择会员但未点选具体会员，**When** 查看确认按钮，**Then** 按钮灰掉不可点。
24. **Given** 散客·加入会员且仅填手机号（11 位）称呼空，**When** 确认核销，**Then** 建档姓名「团购顾客0N」并写入会员列表；成功页展示该名。
24b. **Given** 加入会员路径手机号非 11 位，**When** 点确认核销，**Then** toast「请输入 11 位手机号」。
24c. **Given** 核销明细 / 门店流水列表，**When** 查看团购核销订单，**Then** 主标题为核销团购券名（如「深层补水护理 · 单次体验」）；进详情见匹配项名或「快捷开单」。
