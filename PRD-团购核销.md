...
2. **单店模式**：本期无总部/多分店；**无「业绩归属配置」页**（已删）。核销、授权、订单均按当前登录门店。
...
4. **双平台同屏链路**：抖音与美团共用同一屏幕链（扫码→输码→结果→确认→成功/失败→明细→详情→撤销→匹配/员工）；仅平台文案、种子订单与演示数据不同。扫码页支持 **抖音 / 美团** 平台 Tab；输码与明细 Tab 为 **优惠券（占位）/ 抖音 / 美团**。
5. **授权分平台、相互独立**：抖音走「抖音来客授权」（`authDouyin` / `owner-auth`）；美团授权**独立**于来客，指引页本期为**占位**（`authMeituan` / `owner-auth-mt`，正式页后续补齐）。验券/核销动作按**当前平台**校验对应授权态。
...
14. **演示条**：复用现有演示条；新增美团演示订单；扫码结果演示**尊重当前平台**（抖音/美团）。已删「扫码成功」；保留「识别失败 / 已用过 / 价目未匹配 / 多匹配」；含「价目已匹配」（预写默认券单项 `p21` 匹配记忆）。
...
- 扫码核销（含抖音/美团平台 Tab）、输码验券（优惠券占位 / 抖音可用 / 美团可用）
...
- 抖音授权指引（none/pending/ok/expired）；美团授权指引**占位页**（独立于来客）
...
| 未授权（美团） | 引导美团授权（占位） | 动作拦截 → `owner-auth-mt` 占位页 |
...
团购核销（单店 · 抖音/美团）
 ...
 ├─ owner-auth-mt 美团授权指引（占位）   <!-- #owner-auth-mt -->
...
flowchart LR
  ...
  authMeituan[[美团授权占位]] -.美团动作拦截.-> CONFIRM
```

| 页面/层 | 锚点 id | 导航标题（原型） | 说明 |
|---------|---------|------------------|------|
| 演示首页（占位） | `home` | — | **非真实 APP 首页**；两个核销入口：顶部扫码图标 + 宫格「验券」 |
| 扫码核销 | `scan` | 扫码 | 相机取景 + 抖音/美团平台 Tab + 输入券号 + 验券订单 |
...
| 美团授权（占位） | `owner-auth-mt` | 美团授权 | **占位屏**；正式页后续 |
...
| `meituan` | 美团团购 | 「美团」「美团团购」；授权走 `authMeituan` → `owner-auth-mt`（占位） |
| （优惠券 Tab） | 占位 | 不进入完整验券/核销 |

扫码页、输码页、明细页切换平台 Tab 时更新 `platform`；后续结果/确认/成功等页文案与授权门禁随之切换。
...
### 4.3.2 美团授权 `authMeituan`（独立于来客）

| 状态 | 含义 | 行为（当前平台=美团） |
|------|------|------------------------|
| 非 `ok` | 未授权 / 待确认 / 到期等 | 拦截 → `owner-auth-mt` **占位页** + toast（示意） |
| `ok` | 已授权 | 允许验券/核销 |

- **美团授权与抖音来客授权相互独立**；一侧 `ok` 不影响另一侧。
- `owner-auth-mt` 本期为占位屏（正式步骤后续补齐）；状态卡可示意当前态。
- **进页**（仅看 `rolePerm`）与 **动作**（`rolePerm` + 当前平台对应授权 `===ok`）分层：无权限进页即拦；有权限但未授权可看指引，点验券/核销才拦。
...
原型左侧「扫码结果演示」仅供评审，**不算产品正式规则**；演示结果**尊重当前扫码平台 Tab**（抖音/美团）：
...
**已删除**「扫码成功」演示按钮（与「价目未匹配」等未匹配路径重复）。另有美团演示订单种子供明细/详情演示。
...
- **平台 Tab：抖音 / 美团**（切换更新 `platform`；演示条结果尊重当前 Tab）。
- 取景框 + 提示：「扫描项目核销码，核销后即可分账」。
- 底部「输入券号」→ `input`（须过 `rolePerm`；可带入当前平台）。
- 识别成功闪层「识别成功」后进入验券 loading → `result`（或失败/幂等）；验券前按当前平台校验对应授权。
...
- 平台标识随当前平台：「抖音团购」或「美团团购」。
- 券名 + 标签「未匹配」/「已匹配」；面额；适用门店；有效期；脱敏券码；平台「抖音」或「美团」。
...
- 卡片：名称、金额、时间、操作人、平台标签、状态（**仅**已核销/已撤销；**不展示已退款**）。
...
### 6.13.2 美团授权（占位） <!-- #owner-auth-mt -->

- 标题示意「美团授权」。
- **本期为占位屏**：说明美团授权独立于抖音来客；正式授权步骤与能力勾选后续补齐。
- 可展示当前门店美团授权状态示意（随 `authMeituan`）。
...
| `auth` | 抖音来客：`none` \| `pending` \| `ok` \| `expired` |
| `authMeituan` | 美团授权：`none` \| `pending` \| `ok` \| `expired`（独立于 `auth`） |
...
| 抖音授权查询 | 查询/刷新门店来客授权状态与到期日 | 【待联调确认】 |
| 美团授权查询 | 查询/刷新美团门店授权（独立通道）【待联调确认】 | 【待联调确认】 |
...
验券
  ...
  → 美团且 authMeituan 非 ok → owner-auth-mt（占位）
...
4. **双平台同链**：美团与抖音共用同一屏幕链；仅文案、seed、演示数据不同；扫码页有平台 Tab。
5. **平台动态文案**：当前平台为抖音时用「抖音 / 抖音团购」；美团时用「美团 / 美团团购」。
6. **授权独立**：抖音来客（`auth`/`owner-auth`）与美团（`authMeituan`/`owner-auth-mt`）互不影响；美团指引本期占位。
...
**标签/状态**：未匹配、已匹配、已核销、已撤销、已退款（**仅详情等非列表场景**）、未发起、待确认、已授权、已到期、抖音、美团、抖音团购、美团团购。

**关键提示**：扫描项目核销码，核销后即可分账；多项匹配按门店原价占比分摊计算业绩；未匹配价目时，将按券面额开单；业绩归属未选则该单不计业绩；已自动生成开单，可在验券订单中查看明细；对应开单已冲销，员工业绩已回滚，撤销流水已保留；在抖音来客完成门店授权后，方可在本店核销抖音团购券；美团授权占位提示（以原型为准）；当前账号未开启「收银开单」权限，无法扫码核销…
...
| 美团门店授权 `authMeituan` | 仅 `ok` 可验券/核销（平台=美团）；否则引导 `owner-auth-mt` 占位；**独立于** `authDouyin` |
...
| 抖音授权四态 | auth：none / pending / ok / expired |
| 美团授权态 | authMeituan：至少含非 ok / ok（占位页可演示） |
...
**有权扫码主链路（抖音）**
1. **Given** 有核销权限且抖音门店已授权、联网、相机可用，**When** 从演示首页点扫码（顶部图标 / 宫格「验券」）、扫码页为抖音 Tab 并完成识别验券，**Then** 进入验券结果，可「去核销」→ 确认 →「确认核销」→ 核销成功，并生成开单（支付方式 UI 为「抖音团购」）。

**有权扫码主链路（美团）**
2. **Given** 有核销权限且美团授权 ok、联网、相机可用，**When** 扫码页切美团 Tab 并完成验券核销，**Then** 走与抖音相同屏幕链，支付方式 UI 为「美团团购」。
...
**未授权（分平台）**
5. **Given** 有核销权限但 `auth=none|pending|expired` 且当前平台=抖音，**When** 点立即验券或确认核销，**Then** toast 对应文案并进入 `owner-auth`。
6. **Given** 有核销权限但 `authMeituan` 非 ok 且当前平台=美团，**When** 点立即验券或确认核销，**Then** 进入 `owner-auth-mt` 占位页（不进入抖音来客页）。
...
**优惠券占位 / 美团可用**
18. **Given** 输码或明细 Tab，**When** 点优惠券，**Then** 仅占位视觉，不进入完整核销。
...
**价目已匹配演示（非正式）**
25. **Given** 原型演示条且当前平台为抖音或美团，**When** 点「价目已匹配」，**Then** 默认券写入单项 `p21` 记忆，结果/确认页标签「已匹配」，平台文案与当前 Tab 一致——**仅演示，非产品规则**。
...
> 原型左侧「演示条」（网络/相机/**收银开单权限**切换、扫码结果演示：识别失败 / 已用过 / 价目未匹配 / **价目已匹配** / 多匹配；**无**「扫码成功」；结果尊重当前平台；含美团演示订单）仅供评审演示，**不作为产品正式规格**；正式规则以本文状态机与业务规则为准。
