...
15. **员工选择**：开单式卡片翻转（点客/散客 → 大/中/小工），可多选；照片头像。
16. **核销成功页会员选填（v1.5.7）**：卡片标题「选填」；字段名为「手机号 / 称呼 / 性别」（**无**「（选填）」后缀）。三项仍为选填：全空可点「完成 / 继续核销」离开；**若已填任一项，则须三项齐全且手机号 11 位**，否则：手机号非法 → toast「请输入 11 位手机号」；部分填写 → 弹窗询问是否放弃（「继续填写 / 放弃填写」），放弃后进入原点击入口。三项齐全且称呼+性别有值则自动注册会员。
17. **演示条**：券类型先行（抖音团购券 / 美团团购券 / 系统优惠券 / **代金券** / **抖音次卡** / **美团次卡**），点击即模拟扫码；次卡分别绑定对应平台 L1 次卡目录项。开通演示条（A/B 旁）可切未开通 / 审核中 / 1/3 / 2/3 / 已开通 / 已过期 / 授权兜底。
18. **账号管理入口**：演示首页点**门店名**进入「账号管理」；「团购核销」行右侧抖音/美团分两行热区，展示 **去开通 / 已开通 / 已到期**，点击进入对应 **A/B 开通页**（`auth-open`）。
19. **未开通弹窗**：当前平台未开通时扫码/验券 → **居中弹窗**（非 toast）；「去开通」进 A/B，「取消」关闭留在当前页。
20. **异常态**：原整页异常（相机无权限、断网整页、超时、无核销权限、验券失败、核销失败）改为**居中弹窗**，保留原功能按钮；**既有 toast / 既有弹窗（如离线进扫码提示、已核销幂等）不改动**。
21. **接入身份（v1.6.0）**：**抖音/美团均经汇付天下 ISV**（非抖音直连）。**用户侧开通在 APP 内三步完成**；汇付 API 为后端联调对象（见模块 13）。门店绑定在 APP 多选完成（废绑定码 K1）。ID 分 L1 目录 / L2 订单 / L3 券实例。L1 目录能力**待汇付确认**；原型双平台均有 seed 目录示意。本期不做汇付分账抽佣。
22. **团购设置（v1.5.1+ / v1.6.1）**：工作台入口 → 双平台卡片（**已开通**显示「共 N 项」，N 为已绑门店目录去重合计）→ L1 列表（**多店 TAB**）→ 默认配置按「门店 + L1」存储。非已开通 → A/B；详见 6.13.6。
23. **品牌方标 vs 订单标签（v1.5.1）**：全站抖音/美团**品牌方标**统一为 Figma SVG；核销明细等处的**黄底白字「美团」标签**仍用 `meituan-tag.svg`。
24. **匹配标签移除（v1.5.3）**：验券结果 / 确认核销页**不再展示**「已匹配 / 未匹配」标签。
25. **券类型范围（v1.5.8）**：主链路支持**团购套餐券 + 平台标准次卡**；扣次与「单次核销」项数量解耦；**代金券**仍不支持；系统优惠券走线上能力。
26. **筛选无重置（v1.5.3）**：核销明细 `filterMask` **无「重置」**，仅取消 / 确定。
27. **1C/1D/1E 返回层级（v1.5.4）**：1C→工作台、1D→1C、1E→1D。
28. **支付方式称呼（v1.5.5）**：确认核销页「支付方式」随平台显示 **「抖音」/「美团」**；**平台标识**仍为「抖音团购」/「美团团购」。
29. **1C 卡片标题（v1.5.6）**：团购设置双平台卡片标题「抖音团购」/「美团团购」；开通页标题「抖音开通」/「美团开通」。
30. **核销项目 / 次卡 / 成功页选填 / 流水作废 / 流水商品图（v1.5.7）**：字段「匹配项目」→「核销项目」；次卡可核销；成功页选填规则；撤销后流水作废；流水商品图 48×48。
31. **次卡对齐平台规则（v1.5.8）**：独立「本次核销次数」（默认 1）；结果/确认展示总次数+剩余次数；开单金额=单次结算价×本次次数；美团多扣授权提示（示意）；按次撤销恢复次数。
32. **次卡目录与配置闭环（v1.5.9）**：L1 列表抖音/美团各造 1 张次卡（列表末尾）；淡蓝「次卡」标签；默认配置只读售价/总次数/单次结算价。
33. **次卡选择消费项与团购对齐**：次卡「选择消费项」有数量步进器（1–99）；确认/配置页字段标签为「单次核销」（团购券仍为「核销项目」）；平台扣次仍仅看「本次核销」。
34. **平台开通三步（v1.6.0）**：全面替换旧四步汇付指引；验收见模块 10 / 13。
35. **多店团购券 TAB + 演示数据统一（v1.6.1）**：绑定可选门店与首页/适用门店统一为「悦颜美肌 · 中山路/人民路/高新」；演示默认双平台绑 3 家；各店 L1 券种可不同；`dealDefaults` 按门店分桶；核销预填取**当前登录门店**配置。门店 TAB 样式对齐输码验券；全名 +「 · 」换行；3 项不横滑。
...
| 成功页会员选填 | 卡片标题「选填」；可选填手机号/称呼/性别 | 全空可离开；填任一项须三项齐全；手机号非法 toast；部分填写弹放弃窗；齐全则自动注册会员 |
...
成功页卡片标题「选填」；三行字段：**手机号**、**称呼**、**性别**（男/女分段控件，**无**「（选填）」后缀）。**已删除**独立 `phoneMask` sheet。

| 规则 | 说明 |
|------|------|
| 全空 | 可不填直接点「完成 / 继续核销」离开 |
| 填了任一项 | 须三项齐全且手机号 11 位；否则 toast 或放弃填写弹窗 |
| 手机号有值但不合法 | toast「请输入 11 位手机号」且**不离开**（优先于放弃弹窗） |
| 三项齐全 | 点「完成 / 继续核销」时若称呼+性别有值则**自动注册会员**（写入线上会员列表）；原型 toast「已注册为会员：XXX」示意 |
| 称呼展示名 | 仅 1 个字：男→`X先生`，女→`X小姐`；≥2 字原样 |
| 部分填写（如只填手机号） | **不离开**；弹窗「继续填写 / 放弃填写」；放弃后进入原点击入口（完成→首页 / 继续核销→扫码） |
| 「继续核销」 | 与「完成」同一套选填校验；通过后进入扫码页 |
...
- 未匹配提示条（未匹配时显示）：「未匹配价目时，将按券面额开单；业绩归属未选则该单不计业绩。」（次卡未匹配时按单次结算价×次数开单）
...
- **已开通**：授权账号、已绑定门店（可管理）、「取消授权」、主按钮「团购设置」（含设置 SVG 图标）。
...
**返回层级（v1.5.4）**：1C→工作台、1D→1C、1E→1D。

**首页卡片**：已开通「共 N 项」（N = 当前平台**已绑门店**目录 L1 **去重**合计）；未开通「去开通」；已到期「授权已到期，去续期」。点击：非已开通 → `auth-open`；已开通 → `tg-deals`。

**团购券列表门店 TAB（v1.6.1 · `tg-deals`）**：
- 数据源：当前平台 AuthOpen 已绑定门店。
- **≥2 家**：标题栏下展示横向门店 TAB；切换后刷新该店 L1 列表；进入 `tg-config` 携带当前 `tgStoreId`；返回仍落在该 TAB。
...
**列表与默认配置**：同 v1.5.9–v1.5.10（次卡、核销项目、无页底预填提示；用户端不展示团单/商品 ID）。`dealDefaults` 按「门店 + L1」分桶；核销预填优先取**当前登录门店**下该 L1 的默认匹配。撤销后流水作废规则同 v1.5.7。
...
21. **成功页会员选填**：卡片标题「选填」；全空可离开；填任一项须三项齐全；手机号非法 toast；部分填写弹放弃窗；齐全且称呼+性别有值则注册会员。无 `phoneMask`。
22. **输码演示关键字**（used/fail/mismatch 等）及扫码结果演示条：**仅原型演示，不算产品规则**；演示尊重当前平台；含美团演示订单。
23. **团购设置**：双平台阴影卡片 + Figma 品牌方标；已开通「共 N 项」（已绑门店目录去重）；多店时 `tg-deals` 顶 TAB（同输码验券样式、全名智能换行、3 项不横滑）；列表阴影券卡（分割线 + 已配置/未配置）；默认配置顶卡无页底预填提示；仅核销项目+数量；`dealDefaults[storeId][L1].matches:[{id,qty}]`；核销预填取登录门店桶。
...
**导航/按钮**：扫码、验券订单、输入券号、立即验券、重新扫码、去核销、选择消费项、修改、取消、确认核销、确定、清空、继续核销、完成、筛选、应用、撤销核销、返回核销明细、查看原单、手动输码、查看订单、去团购设置、重试核销、我知道了、项目、产品、优惠券、抖音、美团、点客、散客、大工、中工、小工、男、女。
...
**成功页会员选填**
22. **Given** 核销成功页三项全空，**When** 点「完成」或「继续核销」，**Then** 直接离开，不弹窗。
23. **Given** 已填任一项但未齐全，**When** 点「完成」或「继续核销」，**Then** 弹窗「放弃填写顾客信息？」；「继续填写」关闭弹窗；「放弃填写」清空并进入原入口。
24. **Given** 三项齐全且手机号合法、称呼+性别有值，**When** 点「完成」，**Then** 自动注册会员并离开。
24b. **Given** 已填手机号但非 11 位，**When** 点离开，**Then** toast「请输入 11 位手机号」。
