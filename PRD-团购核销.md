...
- 底部「输入券号」→ `input`（须过 `rolePerm`；可带入当前平台）：自扫码进入时以 **底部 sheet 上滑**呈现（轻回弹，无强挤压；返回下滑收起回扫码）；**sheet 自状态栏下方起，不覆盖顶部状态栏**。
...
- **呈现（v1.6.11 / v1.6.12）**：自扫码「输入券号」进入时覆盖在扫码之上为 sheet，且**不遮挡状态栏**；其它入口仍可整页进入。
...
23. **团购设置**：双平台阴影卡片 + Figma 品牌方标；已开通「共 N 项」（已绑门店目录**按店累加**，非去重）；已开通卡进 `tg-deals` 为**右侧滑入**、返回**右侧滑出**；多店时顶 TAB + 切 Tab stagger；列表阴影券卡**内联展开**默认配置（无重复卡头；次卡仅 kv；隐藏摘要；仅「已配置」下一划线；废弃整页 `tg-config`）；`dealDefaults[storeId][L1].matches:[{id,qty}]`；核销预填取登录门店桶。开通页「去设置团购券」直达 `tg-deals`，返回开通页。
24. **核销明细**：Tab stagger；点卡内联展开详情（无「状态」字段）；**次卡**含总次数/核销次数/剩余次数；展开区可撤销。
25. **扫码→输码**：sheet 上滑轻回弹且**不覆盖状态栏**；返回下滑收起。
26. **品牌资源**：方标 `douyin-logo.svg` / `meituan-logo.svg`；订单黄底「美团」标签仍为 `meituan-tag.svg`。
27. **范围边界**：其它收银/会员能力沿用线上；冲突以原型界面修正 PRD。
---
