...
2. **APP 首页仅为演示占位，并非真实 APP 首页**。原型用占位首页承载扫码按钮，便于演示主链路；**真实产品入口以线上 APP 首页扫码核销入口为准**。宫格「抖音核销」仅作展示，不作为正式入口验收。详见模块 1 / 3 / 6.1。
...
5. **账号无核销权限**：拦截全部核销入口（扫码 / 输码 / 主链路导航 / 重新扫码 / 手动输码等）→「无核销权限」页；验券、核销等动作时仍做兜底拦截。
...
9. **撤销**：核销后 **1 小时内**可撤；超时不可撤；顾客退款后为「退款冲销」态；幂等「已核销」弹窗可查看原单。
...
- 需覆盖账号无核销权限、门店未授权/到期、断网、相机权限、已核销幂等等异常；
...
| 核销明细与撤销 | 列表/详情；1h 内可撤；超时/冲销态正确 |
| 权限与授权门禁 | 无核销权限进页即拦；门店授权未 ok 拦动作 |
...
| 无权限仍可进扫码 | 全部核销入口拦截 → 无核销权限页；动作兜底 |
| 已核销重复扫 | 幂等弹窗，可查看原单 |
| 误核销无法回退 | 1 小时内可撤销；超时不可撤；退款冲销态 |
| 美团同页核销 | Tab 占位，本期不做 |
...
| 角色 | 说明 |
|------|------|
| 店员 | 日常扫码/输码核销（需具备核销权限） |
| 店长 | 核销 + 门店授权协同 |
| 店主 | 门店授权与权限发放 |

本期权限差异：**账号是否具备「抖音核销」操作权限**（`rolePerm`）与 **门店是否已授权**（`auth`）分开判定，见模块 4 / 8 / 9。
...
抖音团购核销（单店）
 ...
 └─ 弹层
      ...
      ├─ phoneMask / filterMask / revokeMask
      └─ loadingMask
```
...
|---------|---------|------------------|------|
| 演示首页（占位） | `home` | — | **非真实 APP 首页**；演示扫码入口 |
...
| 其它弹层 | offline/dup/phone/filter/revoke/loading | — | 见 6.17 |
...
| 状态 | 含义 | 行为 |
|------|------|------|
| `true` | 有核销权限 | 可进入扫码/输码及后续主链路 |
| `false` | 无核销权限 | **全部核销入口** → `tpl-noperm`；验券/核销动作兜底拦截 |
...
|------|------|
| 选填 | 可跳过；跳过 toast「已跳过」 |
| 有值 | 须匹配 `1` 开头 11 位；否则 toast「请输入 11 位手机号」 |
| 保存成功 | 回填脱敏 `138****1234 ▸`；toast「手机号已保存」 |
...
> **本页仅为演示占位，并非真实 APP 首页。** 真实入口以线上 APP 首页扫码核销入口为准；原型用本页承载扫码按钮演示主链路。

- 门店名示意「悦颜美肌 · 旗舰店」。
- **正式演示入口**：顶部右侧扫码图标 → 经门禁进入 `scan`。
- 宫格「抖音核销」**仅展示**，不作为正式入口验收。
...
- 抖音卡片：名称、金额、时间、操作人、平台标签、状态（已核销/已撤销/退款冲销）。
...
| `tpl-timeout` | 超时模板 |
| `tpl-noperm` | **无核销权限**；账号未被授予抖音核销权限时的统一落点 |
...
| `phoneMask` | 登记手机号 | 选填；跳过/保存 |
| `filterMask` | 筛选 | 状态：全部/已核销/已撤销/退款冲销；时间：全部/今日；重置/应用 |
...
11. **rolePerm=false**：拦截全部核销入口 + 动作兜底 → `tpl-noperm`。
12. **auth≠ok**：拦验券/核销动作 → `owner-auth`。
13. **撤销窗口 1 小时**；超时不可撤；退款冲销态不可撤且业绩已回滚示意。
14. **幂等已核销**：不重复开单；弹窗可看原单。
15. **美团/优惠券 Tab**：仅占位，本期不做完整核销。
16. **手机号选填**：11 位；可跳过。
17. **输码演示关键字**（used/fail/mismatch 等）：**仅原型演示，不算产品规则**。
...
**导航/按钮**：扫码、验券订单、输入券号、立即验券、重新扫码、去核销、选择消费项、修改、取消、确认核销、确定、清空、继续核销、完成、登记手机号、跳过、保存、筛选、重置、应用、撤销核销、返回核销明细、查看原单、手动输码、查看订单、查看授权指引、重试核销、项目、产品、优惠券、抖音、美团、点客、散客、大工、中工、小工。

**标签/状态**：未匹配、已匹配、已核销、已撤销、退款冲销、未发起、待确认、已授权、已到期、抖音、抖音团购。

**关键提示**：扫描项目核销码，核销后即可分账；多项匹配按门店原价占比分摊计算业绩；未匹配价目时，将按券面额开单；业绩归属未选则该单不计业绩；已自动生成开单，可在验券订单中查看明细；对应开单已冲销，员工业绩已回滚，撤销流水已保留；在抖音来客完成门店授权后，方可在本店核销抖音团购券。
...
| 维度 | 规则 |
|------|------|
| 账号核销权限 `rolePerm` | 无权限：全部入口 → 无核销权限页；动作兜底 |
| 门店授权 `auth` | 仅 `ok` 可验券/核销；否则引导授权页 |
| 店员/店长/店主 | 角色展示沿用线上；是否授予核销权限由账号配置决定 |
...
**有权扫码主链路**
1. **Given** 有核销权限且门店已授权、联网、相机可用，**When** 从演示首页点扫码并完成识别验券，**Then** 进入验券结果，可「去核销」→ 确认 →「确认核销」→ 核销成功，并生成开单（支付方式 UI 为「抖音团购」）。

**无权限拦截**
2. **Given** `rolePerm=false`，**When** 点首页扫码 / 输码 / 主链路导航 / 重新扫码 / 手动输码，**Then** 均进入「无核销权限」页，不进入扫码或核销。
...
> 原型左侧「演示条」（网络/相机/账号权限切换、扫码结果演示）仅供评审演示，**不作为产品正式规格**；正式规则以本文状态机与业务规则为准。
